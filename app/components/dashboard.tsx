...
export function Dashboard({ market, loading }: Props) {
  ...
  const values = [
    ["Spot", metrics?.spot.value ?? 0, "green", market?.symbol ?? "--", price],
    ...
    ["Zero Gamma", metrics?.zeroGamma.value ?? 0, "yellow", market?.range ?? "0DTE", price],
  ...
  return <div className="stats" aria-busy={loading}>
    {values.map(([label, value, tone, detail, format]) => <article key={label} className="stat"><label>{label}</label><strong className={tone}>{loading ? "--" : format(value)}</strong><small>{detail}</small></article>)}
  </div>;
}
