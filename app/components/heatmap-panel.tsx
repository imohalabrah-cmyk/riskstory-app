...
export function HeatmapPanel({ market, title = "GEX Heatmap by Expiration", onExpand, onSymbol, compact = false }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    return [...byStrike.values()].filter((row) => !query || (Number.isFinite(needle) && Math.abs(row.strike - needle) < 0.01)).sort((a, b) => b.strike - a.strike);
  ...
  const reset = () => { setSide("combined"); setExpiry("all"); setQuery(""); setAutoScale(true); setZoom(1); setScrollTop(0); if (scrollerRef.current) scrollerRef.current.scrollTo({ top: 0, left: 0, behavior: "smooth" }); };
  ...
  const cell = (row: ExposureStrike, item: { expiration: string; rows: ExposureStrike[] }) => {
    ...
    const max = autoScale ? globalMax : expirationMax.get(item.expiration) ?? 1;
  ...
  return <Panel title={title} onExpand={onExpand} className={classNames("heatPanel", "heatmapIntelligence", compact && "heatCompact")}>
    <div className="heatControls">
      ...
      <button type="button" className={classNames("heatToggle", autoScale && "active")} onClick={() => setAutoScale((value) => !value)}><SlidersHorizontal size={14} />Auto scale</button>
    ...
    {!available || !rows.length || !expirations.length ? <div className="surfaceEmpty heatUnavailable"><strong>Heatmap unavailable</strong><span>Provider-backed option-chain rows are not available for this symbol.</span></div> : <>
      ...
      <div className="heatUtility"><label><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} inputMode="decimal" placeholder="Search strike" aria-label="Search strike" /></label><div><button type="button" onClick={() => setZoom((value) => Math.max(.7, Number((value - .1).toFixed(1))))} aria-label="Zoom out"><Minus size={14} /></button><span>{Math.round(zoom * 100)}%</span><button type="button" onClick={() => setZoom((value) => Math.min(1.6, Number((value + .1).toFixed(1))))} aria-label="Zoom in"><Plus size={14} /></button></div></div>
      <div ref={scrollerRef} className="heatViewport" tabIndex={0} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} onPointerDown={(event) => { const node = scrollerRef.current; if (!node || (event.target as HTMLElement).closest("button")) return; dragRef.current = { x: event.clientX, y: event.clientY, left: node.scrollLeft, top: node.scrollTop }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={onDragMove} onPointerUp={() => { dragRef.current = null; }} onPointerCancel={() => { dragRef.current = null; }}>
        <div className="heatMatrix" style={{ "--heat-columns": expirations.length, "--heat-row-height": `${rowHeight}px`, "--heat-width": `${96 + expirations.length * 122}px` } as React.CSSProperties}>
      ...
      <div className="heatFooter"><span>Drag to pan</span><span>Use +/- to zoom</span><span>Hover a cell for its exact open interest</span>{hovered && <strong>{hovered.expiration} · {price(hovered.strike)} · {hovered.value.toLocaleString()} OI</strong>}</div>
    </>}
  </Panel>;
}
