...
function money(v){var s=v<0?'-':'';var a=Math.abs(v);if(a>=1e9)return s+'$'+(a/1e9).toFixed(1)+'B';if(a>=1e6)return s+'$'+(a/1e6).toFixed(1)+'M';if(a>=1e3)return s+'$'+(a/1e3).toFixed(1)+'K';return s+'$'+a.toFixed(0)}
function clsAsset(s){if(['SPX','SPY','QQQ','IWM','DIA','VIX'].indexOf(s)>=0)return'index';if(['XLF','XLE','XLK','XLV','XLY','XLP','XLI','XLU','XLB','XLC','TLT','HYG','LQD','GLD','SLV','USO','TQQQ','SQQQ','SOXL','SOXS','UVXY','SMH','ARKK'].indexOf(s)>=0)return'etf';return'stock'}
var flowData=[];
window.state=state;
