...
function trinityMini(summary){
  ...
  return html+'<button type="button" class="triMiniOpen" data-view-jump="trinity">Open Trinity workspace <span>&#8594;</span></button></div>';
}
...
function trinityCard(read,summary){
  ...
  if(!read.available){
    return '<article class="tri triPro is-unavailable" data-tri-card="'+symbol+'"><header class="triProHead"><div class="triIdentity"><strong>'+symbol+'</strong><div><b>'+(price?price.toFixed(price>1000?0:2):'--')+'</b><span class="'+((snap.provenance&&snap.provenance.mode)||'unavailable')+'">'+readinessModeLabel((snap.provenance&&snap.provenance.mode)||'unavailable')+'</span></div></div><div class="triCardActions"><button type="button" class="triIcon gear" data-tri-settings="'+symbol+'" title="Expiry settings">&#9881;</button></div></header>'+trinityConfig(symbol,read.range)+'<div class="triUnavailableBody"><strong>Provider read unavailable</strong><span>'+(read.reasons&&read.reasons[0]?read.reasons[0]:'No provider-backed chain was returned for this symbol and expiry.')+'</span></div></article>';
  }
  return '<article class="tri triPro'+(read.available?'':' is-unavailable')+'" data-tri-card="'+symbol+'"><header class="triProHead"><div class="triIdentity"><strong>'+symbol+'</strong><div><b>'+(price?price.toFixed(price>1000?0:2):'--')+'</b><span class="'+((snap.provenance&&snap.provenance.mode)||'unavailable')+'">'+readinessModeLabel((snap.provenance&&snap.provenance.mode)||'unavailable')+'</span></div></div><div class="triCardActions"><button type="button" class="triIcon" data-tri-scroll="up" data-tri-target="'+symbol+'" title="Scroll up">&#8593;</button><button type="button" class="triIcon" data-tri-scroll="down" data-tri-target="'+symbol+'" title="Scroll down">&#8595;</button><button type="button" class="triIcon gear" data-tri-settings="'+symbol+'" title="Expiry settings">&#9881;</button></div></header>'+trinityConfig(symbol,read.range)+
...
function loadTrinitySymbol(symbol,range){
  ...
  state.trinityLoading[key]=source.loadMarketRead(symbol,range).then(function(read){delete state.trinityLoading[key];state.renderRevision=Number(state.renderRevision||0)+1;renderTri('triFull');return read}).catch(function(error){delete state.trinityLoading[key];renderTri('triFull');return null});
